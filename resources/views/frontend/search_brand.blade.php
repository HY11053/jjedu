...
@section('title')品牌搜索页面-树人教育加盟网@stop
                ...
                <ul class="xm-list-H224 clearfix" style="width: 870px; float:left; cursor: pointer">
                    @foreach($pagelists as $pagelist)
                        <li class="">
                            ...
                            <a target="_blank" href="/jyproject/{{$pagelist->id}}.shtml" class="img-block magnify"><img src="{{$pagelist->litpic}}" alt="{{$pagelist->brandname}}"></a>
                            <div class="f20">
                                <a target="_blank" href="/jyproject/{{$pagelist->id}}.shtml">{{$pagelist->brandname}}</a>
                            </div>
                            <div class="info">
                    ...
                    @foreach($topbrands as $index=>$topbrand)
                        <dd @if($loop->first)  class="show" @endif>
                            <div class="item">
                                ...
                                <div class="default">
                                    <span class="fr">{{$topbrand->brandnum}}</span><a target="_blank" href="/jyproject/{{$topbrand->id}}.shtml" class="f16">{{$topbrand->brandname}}</a>
                                </div>
                                <div class="tips">
                                    <a target="_blank" href="/jyproject/{{$topbrand->id}}.shtml"><img src="{{$topbrand->litpic}}" alt="{{$topbrand->brandname}}" class="fr"></a>
                ...
                <div class="bg-ff p20">
                    ...
                    <ul class="join-project mt30">
                        @foreach($flashlingshibrands as $flashlingshibrand)
                            <li>
                                <a target="_blank" href="/jyproject/{{$flashlingshibrand->id}}/" class="img-block magnify"><img src="{{$flashlingshibrand->litpic}}" alt="{{$flashlingshibrand->brandname}}"></a>
                                <p class="f16"><a target="_blank" href="/jyproject/{{$flashlingshibrand->id}}shtml">{{$flashlingshibrand->brandname}}</a></p>
